...
def add_datasources(api_key):
    ...
    # dbmon-star datasource by dbmon
    if 'DBMON-STAR' not in ds:
        print ' * dbmon-star Data Source'
        data = json.dumps({
            'name': 'DBMON-STAR',
            'type': 'mysql',
            'url': '127.0.0.1:3306',
        ...
        print r.status_code, r.content
        if r.status_code != 200:
            print ' * Cannot add dbmon-star Data Source'
            sys.exit(-1)
...
def copy_apps():
    for app in ['pmm-app']:
        ...
        dest_dir = GRAFANA_DB_DIR + '/plugins/' + app
